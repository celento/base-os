...
d=pathlib.Path(tempfile.mkdtemp(prefix='baseos-ui-'));print(d,flush=True)
def tool(n):return 'x86_64-elf-'+n if shutil.which('x86_64-elf-'+n) else n
subprocess.run([tool('gcc'),'-Os','-ffreestanding','-m32','-fno-pie','-fno-stack-protector','-fno-builtin','-mno-sse','-mno-mmx','-msoft-float','-I',str(root),'-I',str(root/'src'),'-I',str(build),'-c',str(root/'tests/ui_guest.c'),'-o',str(d/'kernel.o')],check=True)
...
run(d/'disk.img',d,'desktop-features','UI-FEATURES-PASS',seconds=45)
run(d/'disk.img',d,'session-reboot','SESSION-RESTORE-PASS',seconds=30)
print('Desktop features and session recovery passed.',flush=True)
